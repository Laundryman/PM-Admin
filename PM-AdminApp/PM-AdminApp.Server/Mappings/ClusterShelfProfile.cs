...
{
    public class ClusterShelfProfile : Profile
    {
        public ClusterShelfProfile()
        {
            CreateMap<ClusterShelf, PlanmPartInfo>()
                .ForMember(pm => pm.Id, opt => opt.MapFrom(src => src.Id))
                .ForMember(pm => pm.Position, opt => opt.MapFrom<ClusterShelfPositionResolver>())
                .ForMember(pm => pm.ClusterId, opt => opt.MapFrom(src => src.ClusterId))
                .ForMember(pm => pm.ClusterShelfId, opt => opt.MapFrom(src => src.Id))
                .ForMember(pm => pm.ClusterId, opt => opt.MapFrom(src => src.ClusterId))
                ...
                .ForMember(pm => pm.PlanogramPartId, opt => opt.Ignore())
